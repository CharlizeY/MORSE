...
# compute the log-likelihood using expression ell(q,theta), p34, 36, 38
...
# compute the log-likelihood directly-slides p27
...
em_mix_gaussian <- function(xs,K,max.numit=Inf) {
  ...
  ## start off with a random selection of cluster mean, sampled from xs
  mus <- xs[sample(n,K),]
  
  ## let the initial covariances be simple estimates of the variance-with diagonal of all variances and the rest 0
  vs <- rep(0,p)
  ...
  while(!converged && numit < max.numit) {
    ...
    # M step - update ws, mus and Sigmas, p39-40 (then in the loop gamma is updated too)
    Ns <- rep(0,K)
    for (k in 1:K) {
      Ns[k] <- sum(gammas[,k])
